...
coffee_long <- coffee_long %>%
  mutate(participant_id = rep(1:nrow(coffee_cleaned), each = 3))
...
# Branding effect: Flour - Dunkin
branding_effect <- fixef(model3)["coffee_typeflour"]
cat("Branding effect (Flour - Dunkin):", branding_effect, "\n")

# Taste effect: Swissbakers - Dunkin  
taste_effect <- fixef(model3)["coffee_typeswissbakers"]
cat("Taste effect (Swissbakers - Dunkin):", taste_effect, "\n")
```
...
coffee_long <- coffee_long %>%
  left_join(coffee_cleaned %>% 
              mutate(participant_id = row_number()) %>%
              select(participant_id, cupsperweek),
            by = "participant_id")
